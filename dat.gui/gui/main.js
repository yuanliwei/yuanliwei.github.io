...
define('dat/utils/css',{
  ...
  link: function(url, doc) {
    ...
    link.disable = function() {
      head.removeChild(link);
    };

    var _onLoad = function() {};
    link.onLoad = function(fnc) {
      _onLoad = fnc;
    };

    (function(s) {
      if (document.styleSheets.length != s) {
        _onLoad();
      } else
  ...
  inject: function(css, doc) {
    doc = doc || document;
    var injected = doc.createElement('style');
    injected.type = 'text/css';
    injected.innerHTML = css;
    var head = doc.getElementsByTagName('head')[0];

    head.disable = function() {
      head.removeChild(injected);
    };
...
define('dat/dom/dom',[
  ...
  /**
   * For css-like hover interactivity. Snagged from jQuery
   */
  var special = {
  ...
  var dom = {
    ...
    makeSelectable: function(elem, selectable) {
      ...
      var selectionStyle = selectable ? 'auto' : 'none';

      elem.style.MozUserSelect = selectionStyle;
      elem.style.KhtmlUserSelect = selectionStyle;
      elem.style.webkitUserSelect = selectionStyle;
      elem.style.msUserSelect = selectionStyle;
    ...
    bind: function(elem, event, func, bool) {
      ...
      if (!elem) {
        // console.log('no elem', arguments.callee.caller);
        return;
      }
    ...
    unbind: function(elem, event, func, bool) {
      ...
      if (event in special) {
        ...
        event = special[event];
        var _func = specialEvents[func];  // Fetch me my mouse event.
        if (common.isFunction(_func)) {
          func = _func;
          delete specialEvents[func];
        }
      }
    ...
    getRect: function(elem, excludeMargin) {
      var offset;
      if (elem.getBoundingClientRect) {
        offset = elem.getBoundingClientRect();
      } else {
        offset = this.getOffset(elem);
        var width = this.getWidth(elem);
      ...
      if (excludeMargin) {
        ...
        var mt = cssValueToPixels(style['margin-top'] || style.marginTop);
        var ml = cssValueToPixels(style['margin-left'] || style.marginLeft);
        offset.top -= mt;
        offset.left -= ml;
    ...
    getWidth: function(elem) {
      ...
      return cssValueToPixels(style['border-left-width'] || style.borderLeftWidth) +
          cssValueToPixels(style['border-right-width'] || style.borderRightWidth) +
          cssValueToPixels(style['padding-left'] || style.paddingLeft) +
          cssValueToPixels(style['padding-right'] || style.paddingRight) +
    ...
    getHeight: function(elem) {
      ...
      return cssValueToPixels(style['border-top-width'] || style.borderTopWidth) +
          cssValueToPixels(style['border-bottom-width'] || style.borderBottomWidth) +
          cssValueToPixels(style['padding-top'] || style.paddingTop) +
          cssValueToPixels(style['padding-bottom'] || style.paddingBottom) +
...
define('dat/utils/urlArgs',[
  ...
  urlArgs.getBoolean = function(name, defaultValue) {
  ...
  urlArgs.getFloat = function(name, defaultValue) {
  ...
  urlArgs.getInt = function(name, defaultValue) {
  ...
  urlArgs.getHash = function() {
    return !!location.hash ? location.hash.substr(1) : location.hash;
  };

  urlArgs.refresh = function(map, _base) {
    var str = [];
    _.extend(og, map);
    _.each(og, function(value, key) {
      str.push(key + '=' + value);
    });
    window.location.href = (_base || base) + '?' + str.join('&');
  };

  urlArgs.update = function() {
    ...
    urlArgs.result = result;

  };

  urlArgs.get = function(key) {
    this.update();
    return result[key];
  };
...
define('dat/utils/System',[
  ...
  function getVersion() {
    ...
    return { full: fullVersion, major: majorVersion };
...
define('dat/slides/Slides',[
  ...
  'dat/utils/System',
  'underscore'
], function(css, styleSheet, dom, RAF, datEvents, urlArgs, System) {

  var Slide = function(html, update, width) {
    ...
    dom.addClass(this.domElement, 'slide');
    this.domElement.width = this.domElement.style.width = width;
    this.update = update;

  };

  var Slides = function(prms) {

    var _this = this;

    var params = _.defaults(prms || {}, {
      css: '',
      container: document.body,
      transition: function(active) {
        // animate to active div
        _.extend(_this.domElement.firstChild.style, {
          marginLeft: (-dom.getWidth(_this.domElement) * _this.cid()) + 'px'
        });
      },
      loopKeys: false,
      onChange: _.identity,
    ...
    this.__loopKeys = params.loopKeys;

    css.inject(params.css || styleSheet);

    var domElement = this.domElement = document.createElement('div');
    dom.addClass(domElement, 'dat-slides-container');
    domElement.innerHTML = '<ul class="slides"></ul>';
    params.container.appendChild(domElement);

    dom.makeFullscreen(domElement); // TODO: Parameterize

    var deck = this.deck = [];
    var urlMap = this.urlMap = [];
    var onLoadUrl = this.onLoadUrl = window.location.hash;
    ...
    dom.bind(window, 'keyup', function(e) {
      if (document.activeElement.type === 'text') {
        return;
      }
      switch (e.which) {
        case 37:
          if (!_this.__loopKeys && _this.cid() == 0) return;
          _this.prev();
          break;
        case 39:
          if (!_this.__loopKeys && _this.cid() == _this.deck.length - 1) return;
          _this.next();
          break;
      }
    });

    this.transition = function(active) {
      // wrap other stuff here
      params.transition.call(active);
    };
    this.transitioning = false;
    this.onChange = params.onChange;

    this.setRAF = function(raf){
      RAF = raf;
    }

    var afterResize = _.debounce(function() {
      dom.removeClass(_this.domElement.firstChild, 'resize');
    }, 500);

    dom
        .bind(window, 'popstate', function(e) {
      var loc = window.location.hash.toString();
      for (var i = 0, l = _this.urlMap.length; i < l; i++) {
        if (loc === _this.urlMap[i]) {
          _this.activate(i, true);
          break;
        }
      }
    })
        .bind(window, 'resize', function(e) {
          var width = dom.getWidth(_this.domElement);
          var height = dom.getHeight(_this.domElement) - navHeight(_this);
          dom.addClass(_this.domElement.firstChild, 'resize');
          _.extend(_this.domElement.firstChild.style, {
            marginLeft: (-width * _this.cid()) + 'px',
            width: width * _this.deck.length + 5000 + 'px'
          });
          _.each(_this.deck, function(slide) {
            _.extend(slide.domElement.style, {
              overflowX: 'auto',
              height: height + 'px',
              width: width + 'px'
            });
    ...
    function update() {
      RAF(update);
      if (!_.isUndefined(_this.active) && _.isFunction(_this.active.update)) {
        _this.active.update();
      }
    }
  ...
  _.extend(Slides.prototype, datEvents, {
    ...
    activate: function(index, silent) {
      ...
      this.active = this.deck[index];
      this.transition(this.active);

      updateDesc(this);

      if (history && !silent) {
//        history.pushState({ index: index }, '', this.urlMap[index]);
        window.location.hash = this.urlMap[index];
      }
    ...
    /**
     * Adds a slide to our deck
     */
    add: function(html, update) {

      addSlide(this, html, update);
    ...
    /**
     * Return index of given slide.
     */
    indexOf: function(slide) {

      return _.indexOf(this.deck, slide);

    },

    next: function() {
      var id = (this.cid() + 1) % this.deck.length;
      this.activate(id);
    },
  ...
  function injectUI(_this) {

    var navContainer = _this.navContainer = document.createElement('div');
    var navi = document.createElement('ul');

    dom
        .addClass(navContainer, 'navigation-container')
        .addClass(navi, 'navigation')
        .makeSelectable(navContainer, false);

    var desc = _this.desc = document.createElement('li');
    dom.addClass(desc, 'desc');
    desc.setAttribute('id', 'dat-slides-desc');
    desc.innerHTML = '<span></span><div id="dat-slides-toc"></div>';

    var next = _this._next = document.createElement('li');
    next.innerHTML = '<span id="s1-next-slide"></span><a>&rarr;</a>';
    var prev = _this._previous = document.createElement('li');
    prev.innerHTML = '<a>&larr;</a><span id="s1-prev-slide"></span>';

    navi.appendChild(prev);
    navi.appendChild(desc);
    navi.appendChild(next);

    dom
        .addClass(next, 'next')
        .addClass(prev, 'prev')
        .addClass(_this.domElement, 'auto-ui')
    ...
    navContainer.appendChild(navi);
    _this.domElement.appendChild(navContainer);

  }

  function updateDesc(_this) {
    if (_.isElement(_this.desc)) {
      _this.desc.firstChild.innerHTML = (_this.cid() + 1) + '\/' + _this.deck.length;
    }
    _this.onChange.call(this);
  }

  function addSlide(_this, html, update) {

    var width = dom.getWidth(_this.domElement);
    var height = dom.getHeight(_this.domElement) - navHeight(_this);
    var slide = new Slide(html, update, width);
    ...
    var i = _this.deck.length;
    var title = (html.querySelector('h1').innerHTML || ('slide ' + i));
    _this.urlMap.push('#' + title.replace(/[\ \W\s]/g, '-'));
    _this.deck.push(slide);
    _this.domElement.firstChild.appendChild(slide.domElement);

    // Set active slide
    if (_this.deck.length <= 1) {
      _this.activate(0);
    } else if (_this.onLoadUrl === _this.urlMap[i]) {
      _this.activate(i);
    }
  ...
  function navHeight(_this) {
    if (System.browser === 'Firefox') {
      return 0;
    }
    return (_this.navContainer ? dom.getHeight(_this.navContainer) : 0);
  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var GUI = function GUI(pars) {
	  ...
	  /** @lends dat.gui.GUI.prototype */
	  {
	    ...
	    /**
	     * Determines whether or not to use <a href="https://web.archive.org/web/20200717112037/https://developer.mozilla.org/en/DOM/Storage#localStorage">localStorage</a> as the means for
	...
	function addSaveMenu(gui) {
	  ...
	  _dom2.default.bind(button2, 'click', function () {
	    ...
	    if (presetName) {
	      gui.saveAs(encodeURIComponent(presetName));
	    }
	  });
...
// processing defaults
      var noiseProfile = { generator: undefined, octaves: 1, fallout: 0.5, seed: undefined};
      var generator = new PerlinNoise(noiseProfile.seed);
      var effect, k, sum, i, l = noiseProfile.octaves;
      return generator.noise2d;

    });
define('dat/types/ExtendableFloatArray',[
], function() {
...
define('workshop/FizzyText',[
  ...
  var FizzyText = function(message, w, h, isDark, fontSize) {
    ...
    s.font = g.font = "bold " + fontSize + "px Helvetica, Arial, sans-serif";
    ...
    var height2 = this.height2 = height / 2, fontSize2 = this.fontSize2 = fontSize / 2;
...
define('dat/google/webfont/loader',[
  ...
  return function(fonts) {
    ...
    _.extend(window.WebFontConfig, fonts);
...
define('examples/utils/utils',[
  ...
  return {
    ...
    addTableOfContents: function(elem, slides) {

      var ul = document.createElement('ul');
    
      _.each(slides.domElement.getElementsByClassName('slide'), function(slide, i) {
        
        var text = slide.innerHTML.match(/\<h1\>(.*)\<\/h1\>/)[1];
        var href = '#' + text.replace(/[\ \W\s]/g, '-');
      ...
      var h = dom.getHeight(elem); // TODO
      elem.style.height = '0px';
      elem.style.zIndex = '1001';

      dom
        .bind(elem.parentNode, 'mouseover', function(e) {
          elem.style.height = h + 'px';
        })
        .bind(elem.parentNode, 'mouseout', function(e) {
          elem.style.height = '0px';
        });

    },

    addBackButton: function(elem) {

    var container = document.createElement('li');
    container.innerHTML = '<a id="back" href="/">back to workshop</a>';

    elem.insertBefore(container, elem.firstChild);

    },

    makeFooter: function(i, contents, slides) {

      var prev = contents[utils.wrap(i - 1, contents.length)];
      var next = contents[utils.wrap(i + 1, contents.length)];

      var pmsg = prev.match(/\<h1\>(.*)\<\/h1\>/)[1] || '';
      var nmsg = next.match(/\<h1\>(.*)\<\/h1\>/)[1] || '';

      var div = document.createElement('div');
      dom.addClass(div, 'nav');

      if (i > 0) {
        var preva = document.createElement('a');
        preva.innerHTML = pmsg;
        preva.setAttribute('href', '#');
        dom.bind(preva, 'click', function(e) {
          e.preventDefault();
          slides.prev();
          return false;
        });
        dom.addClass(preva, 'prev');
        div.appendChild(preva);
      }

      if (i < contents.length - 1) {

        var nexta = document.createElement('a');
        nexta.innerHTML = nmsg;
        nexta.setAttribute('href', '#');
        dom.bind(nexta, 'click', function(e) {
          e.preventDefault();
          slides.next();
          return false;
        });
        dom.addClass(nexta, 'next');
 ...
 * <a href="https://web.archive.org/web/20200717112037/http://google-code-prettify.googlecode.com/svn/trunk/README.html#langs">README</a>
...
require([
  'dat/slides/Slides',
  'dat/dom/dom',
  'text!examples/gui/contents.html',
  'dat/gui-0.6.0/GUI',
  'workshop/FizzyText',
  'dat/utils/utils',
  ...
  'underscore',
  'prettyprint'
], function(Slides, dom, contents, GUI, FizzyText, utils, webFontsLoader, workshop) {
  ...
  // Handles a full blown web page
  var contents = contents
      .split('\<article\>')
  contents.splice(0, 1);
  contents[contents.length - 1] =
      contents[contents.length - 1].replace(/\<\/body\>(\n|\r\n)\<\/html\>/, '')
  ...
  slides.domElement.style.visibility = 'hidden';

  var initialText = [];
  initialText[1] = 'constrain';
  initialText[2] = 'folders';
  initialText[3] = 'colors!';
  initialText[4] = 'saving';

  initialText[6] = 'events';

  _.each(contents, function(example, i) {

    var param = { autoPlace: false, resizable: true, scrollable: true, hideable: true };
    if (i == 5) {
      param.load = {
        "preset": "Flow",
        "remembered": {
    ...
    guis.push(new GUI.GUI(param));

    var f = new FizzyText(initialText[i] || 'dat.gui', 600, 150, true, 100);
    f.maxSize *= 0.8;
    fizs.push(f);
    ...
    _.extend(gui.domElement.style, {
      position: 'absolute',
      width: GUI.DEFAULT_WIDTH + 'px',
      top: '0px',
      right: '20px'
    });

    var elem = document.createElement('div');
    dom.addClass(elem, 'content');


    elem.innerHTML = example;
    elem.appendChild(workshop.makeFooter(i, contents, slides));
    elem.insertBefore(fiz.domElement, elem.firstChild);
    elem.insertBefore(gui.domElement, fiz.domElement);

    switch (i) {

      case 0:
        gui.add(fiz, 'message');
        gui.add(fiz, 'speed', -5, 5);
        gui.add(fiz, 'displayOutline');
        gui.add(fiz, 'explode');
        break;
      case 1:
        fiz.noiseStrength = 10;
        fiz.growthSpeed = 0.2;
        fiz.maxSize = 6;
        fiz.speed = 0.1;
        gui.add(fiz, 'noiseStrength').step(5);
        gui.add(fiz, 'growthSpeed', -5, 5); // Increment amount
        gui.add(fiz, 'maxSize').min(0).step(0.25); // Mix and match
        gui.add(fiz, 'message', [ 'constrain', 'pizza', 'chrome' ]); // Choose from accepted values
        gui.add(fiz, 'speed', { Stopped: 0, Slow: 0.1, Fast: 5 }); // Choose from named values
        break;
      case 2:
        fiz.growthSpeed = 0.2;
        fiz.maxSize = 10;
        ...
        var f1 = gui.addFolder('Flow Field');
        f1.add(fiz, 'speed');
        f1.add(fiz, 'noiseStrength');
        var f2 = gui.addFolder('Letters');
        f2.open();
        f2.add(fiz, 'growthSpeed');
        f2.add(fiz, 'maxSize');
        f2.add(fiz, 'message');
        break;
      case 3:
        ...
        fiz.color2 = '#fff000';
        fiz.color3 = '#000000';
        gui.addColor(fiz, 'color0');
        gui.addColor(fiz, 'color1');
        gui.addColor(fiz, 'color2');
        gui.addColor(fiz, 'color3');
        break;
      case 4:
        gui.remember(fiz);
        gui.add(fiz, 'message');
        gui.add(fiz, 'speed', 0, 4);
        gui.add(fiz, 'growthSpeed', 0, 1);
        gui.add(fiz, 'noiseStrength', 0, 170);
        gui.add(fiz, 'maxSize', 0, 10);
        break;
      case 5:
        gui.remember(fiz);
        gui.add(fiz, 'message');
      ...
      case 6:
        var c = function(v) {
          alert("The new value is " + v);
        };
        gui.add(fiz, 'maxSize', 0, 10).onFinishChange(c);
        break;
      case 7:
        gui.add(fiz, 'message');
        gui.add(fiz, 'speed', 0, 5);
        gui.domElement.style.cssText = 'position: absolute; -moz-transform: rotate(20deg);    -moz-transform-origin: 60% 100%;    -webkit-transform: rotate(20deg);    -webkit-transform-origin: 60% 100%;    -o-transform: rotate(20deg);     -o-transform-origin:60% 100%;    -ms-transform: rotate(20deg);    -ms-transform-origin: 60% 100%;    transform: rotate(20deg);    transform-origin: 60% 100%;';
        break;
      case 8:
        gui.add(fiz, 'noiseStrength', 0, 100).listen();
        update = _.compose(update, function() {
          fiz.noiseStrength = Math.cos(Date.now() / 1000) * 50 + 50;
        });
        break;
      case 9:
        gui.add(fiz, 'noiseStrength', 0, 100).listen();
    ...
    fiz.domElement.setAttribute('id', 'fizzy-' + i);

    slides.add(elem, update);


  });
//
  var givesyou = document.createElement('div');
  _.extend(givesyou.style, {
  ...
  var firstSlide = slides.domElement.firstElementChild.firstElementChild;

  firstSlide.insertBefore(givesyou, firstSlide.firstElementChild);

  addDownloadLink(document.body);
  workshop.addBackButton(slides.navContainer.firstChild);
  workshop.addTableOfContents(document.getElementById('dat-slides-toc'), slides);

  _.delay(function() {
    slides.domElement.style.visibility = 'visible';
    prettyPrint();
  }, 250);

  function addDownloadLink(elem) {

    var container = document.createElement('div');
    container.setAttribute('id', 'downloads');
    dom.addClass(container, 'last');
    dom.makeSelectable(container, false);
    container.innerHTML =
        '<ul><li class="source"><a href="https://web.archive.org/web/20200717112037/https://github.com/dataarts/dat.gui">Source</a></li><li class="min-dot-js"><a href="https://web.archive.org/web/20200717112037/https://raw.github.com/dataarts/dat.gui/master/build/dat.gui.min.js">dat.gui.min.js</a></li><li class="dot-js"><a href="https://web.archive.org/web/20200717112037/https://raw.github.com/dataarts/dat.gui/master/build/dat.gui.js">dat.gui.js</a></li></ul>';

    elem.appendChild(container);

    // 121px
    var slides = elem.getElementsByClassName('slides')[0];
    var height = dom.getHeight(container);
    _.each(slides.children, function(slide) {

      var content = slide.getElementsByClassName('content')[0];
...
/*
     FILE ARCHIVED ON 11:20:37 Jul 17, 2020 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 01:01:39 Jul 15, 2021.
...
playback timings (ms):
  ...
  LoadShardBlock: 78.899 (3)
  PetaboxLoader3.datanode: 97.508 (4)
  captures_list: 114.014
  CDXLines.iter: 21.626 (3)
  exclusion.robots.policy: 0.189
  exclusion.robots: 0.202
  esindex: 0.014
  PetaboxLoader3.resolve: 93.199
  load_resource: 139.961
*/